...
import os
import argparse
from videoproc import youtube_to_audio
from advanced import result_multi
from splitter import split

def transcribe_basic(args):
    start = time.time()
    ...
    if args.yttranscribe != None:
        if args.ytclipvideo:
            ...
            file_in = youtube_to_audio(args.yttranscribe[0], start_time, end_time)
        else:
            file_in = youtube_to_audio(args.yttranscribe[0])
...
def main():
    ...
    if args.multiproc:
        transcribe_advanced(args)
    else:
        transcribe_basic(args)
